...
.DESCRIPTION
    ...
    This script performs diagnostic operations that may trigger antivirus software:
    - Network connectivity tests (TCP/UDP/HTTPS to external IPs)
    - Service enumeration and status checks
    - Registry reads for proxy and TLS configuration
    - Filesystem permission tests (creates/deletes test files)
    - Certificate store enumeration
    - Process and network connection enumeration
    
    All operations are READ-ONLY except for:
    - Creating temporary diagnostic output folder
    - Creating test files in target directories (immediately deleted)
    - Creating ZIP archive of diagnostic output
    
    This script does NOT:
    - Modify system configuration
    - Install or download software
    - Persist any changes to the system
    - Exfiltrate data (all results stored locally)
...
.NOTES
    ...
    - Uses only native Windows .NET classes
    - No external dependencies or downloads
    - Administrator rights recommended but not required
#>
...
# ERROR HANDLING: Continue on errors to complete all tests even if some fail
...
# ============================================================================
# This section sets up global variables and configuration for the audit.
# No system modifications occur here - only variable initialization.
...
# AV NOTE: Reading current user identity and environment variables is standard practice
# but may be flagged by behavioral analysis. This is READ-ONLY detection.
$script:Env = @{
...
# AV NOTE: Creating folders in script directory is legitimate diagnostic behavior.
# Folder name includes hostname and timestamp for uniqueness.
$script:OutputFolder = Join-Path $PSScriptRoot "warp_audit_$($script:Env.Hostname)_$($script:Env.DateStamp)".ToLower()
$script:CreatedDirs = @()  # Track directories we create for cleanup
...
# ============================================================================
# These functions manage test results and statistics.
# Pure data manipulation - no system interaction.
...
# ============================================================================
# This section tests write permissions to directories where WARP will install.
# AV WARNING: Creating/deleting test files may trigger behavioral detection.
# These are temporary test files created and immediately deleted - no persistence.
# Folder creation may also trigger alerts but is necessary for installation testing.
...
foreach ($pathInfo in $paths) {
    ...
    try {
        # AV WARNING: Creating directories in Program Files may trigger alerts
        if (-not $existed) {
            New-Item -Path $pathInfo.Path -ItemType Directory -Force -ErrorAction Stop | Out-Null
            $script:CreatedDirs += $pathInfo.Path  # Track for cleanup
        }
        
        # AV WARNING: Creating/deleting files is a permission test only
        # File is created with random name and immediately deleted - no persistence
...
# AV WARNING: Creating/deleting registry keys may trigger behavioral detection
# This tests write access to certificate store - key is immediately deleted
$certStore = "HKLM:\SOFTWARE\Microsoft\SystemCertificates"
$testKey = Join-Path $certStore "WARP_Test_$(Get-Random)"

try {
    # Create temporary registry key to test write access
    New-Item -Path $testKey -Force -ErrorAction Stop | Out-Null
    # Immediately delete the test key - no persistence
...
# AV NOTE: Executing netsh.exe to query proxy settings - READ-ONLY operation
try {
...
# AV NOTE: Opening certificate stores in READ-ONLY mode to count certificates
# This is diagnostic only - no modifications to certificate store
try {
    ...
    foreach ($storeName in $certStores) {
        ...
        $store.Open("ReadOnly")  # READ-ONLY access
...
# AV NOTE: The following commands enumerate system state for diagnostics
# All output is written to local files - no network transmission
$diagnostics = @(
...
$secondaryDiagnostics = @(
    ...
    @{Label="netstat_ano.txt"; Cmd="netstat -ano"},  # AV: Process + network enumeration
    @{Label="arp_table.txt"; Cmd="arp -a"},
    @{Label="dns_cache.txt"; Cmd="ipconfig /displaydns"},
    @{Label="firewall_rules.txt"; Cmd="netsh advfirewall firewall show rule name=all"},  # AV: Security config read
    @{Label="wlan_profiles.txt"; Cmd="netsh wlan show profiles"},
    @{Label="network_adapters.txt"; Cmd="Get-NetAdapter | Format-List"},
    @{Label="tcp_connections.txt"; Cmd="Get-NetTCPConnection | Select-Object LocalAddress,LocalPort,RemoteAddress,RemotePort,State,OwningProcess | Format-Table -AutoSize"},  # AV: Connection enumeration
    @{Label="udp_endpoints.txt"; Cmd="Get-NetUDPEndpoint | Select-Object LocalAddress,LocalPort,OwningProcess | Format-Table -AutoSize"},  # AV: Connection enumeration
...
# CLEANUP
# ============================================================================
# This section removes temporary test artifacts and creates a ZIP archive.
# AV WARNING: Deleting folders and creating ZIP files may trigger alerts.
# Only test directories created during permission tests are removed.
# ZIP creation is for convenient transport of diagnostic files - no encryption.

Write-Host "Cleaning up test artifacts..." -ForegroundColor Yellow

# Remove test directories we created during permission tests
# AV NOTE: Only deleting folders we explicitly created (tracked in $script:CreatedDirs)
...
# Create ZIP archive of diagnostic output
# AV WARNING: ZIP file creation may trigger behavioral detection
# This is standard practice for bundling diagnostic files - no malicious intent
$zipCreated = $false
...
try {
    ...
    # Use .NET compression to create ZIP archive
